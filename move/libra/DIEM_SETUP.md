...
# Developer Setup in diem/diem repository
## To setup the diem/diem repository:
* You will need to ensure that you have a stable connection to the internet.
* Git must be installed on your system.
* It is necessary to install the latest version of Rust:
<br>
...
### Steps:
1. Open Diem's github repository at
``
https://github.com/diem/diem
``
2. Create a fork of the Diem repository under your github account by clicking the Fork button on the top right corner of the page.
3. In your forked repository, click on code button and copy the ``HTTPS address`` provided.


4. Now, open your terminal in Visual Code Studio.
5. Run the following command to clone the diem repository -

    * `git clone https://github.com/diem/diem.git && cd diem `

     * This command will clone the repository and create a new folder called "diem".
* You can now start working with the Diem codebase on your local machine.
 ...
 * This document will guide you through executing your first transaction on the Diem Blockchain's testnet.
...
* Build the Diem CLI client and connect to the testnet.
* Create accounts for Alice and Bob.
* Add Fake Diem Coins to Alice’s and Bob’s accounts.
* Submit a transaction.


# Steps to Clone and build Diem Core :
...
2. Checkout the testnet branch-
    ...
    ```
3. Install dependencies To setup Diem Core, change to the diem directory and run the setup script to install the dependencies, as shown below:
    ```
...
# Steps to Build Diem CLI client and connect to the testnet :
...
The sequence number for an account indicates the number of transactions that have been sent from that account. It is incremented by one every time a transaction sent from that account is executed and stored in the Diem Blockchain.
...
If your account “mint” command did not submit your request successfully, refer to Troubleshooting.
...
````
>You can use the command ``query txn_acc_seq 0 0 true`` (transaction by account and sequence number) to retrieve the information about the transaction you just submitted. The first parameter is the local index of the sender account, and the second parameter is the sequence number of the account. To see a sample output of this command refer to Sample Outputs.

>You just submitted your transaction to a validator node on testnet, and it was included in the Mempool of the validator. This doesn't necessarily mean your transaction has been executed. In theory, if the system were slow or overloaded, it would take some time to see the results, and you may have to check multiple times by querying the accounts. To query an account with index 0, you can use the command ``query account_state 0``. The expected output is shown in the Sample Outputs section

To troubleshoot the transfer command, refer to [Troubleshooting](https://developers.diem.com/docs/tutorials/tutorial-my-first-transaction/#the-transfer-command[).
...
Refer to [Life of a transaction](https://developers.diem.com/docs/transactions/basics-life-of-txn/)  for an understanding of the lifecycle of a transaction from submission to execution and storage.
...
- The sequence number of 0 for Bob’s account (index 1) indicates that no transaction has been sent from Bob’s account so far. Every time a transaction is sent from an account, the sequence number is incremented by 1.
<br>
<br>
...
To check the final balance in both accounts, query the balance again for each account as you did in this step. If your transaction (transfer) executed successfully, you should see 100 fake Diem Coins in Alice’s account and 62 fake Diem Coins in Bob’s account.
````
diem% query balance 0
...
````
  diem% account mint 0 110 XUS
  >> Minting coins
  [ERROR] Error minting coins: Server unavailable, please retry and/or
  check **if** host passed to the client is running`
````
...
* To check if an account exists, query the account state. For an account with index 0 enter this: ``diem% query account_state 0``
...
<br>
<u>
<h2> Sample outputs of additional query commands
</u>
